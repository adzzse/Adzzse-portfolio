...
// Grid index helpers
export function toIndex(x: number, y: number, cols: number) {
  return y * cols + x
}
...
export function* search({ cols, rows, grid, start, end, mode = 'dijkstra' }: {
  ...
  const heap = new MinHeap<{ x: number; y: number; g: number; f: number }>((n) => n.f)
...
export function reconstructPath(prev: Int32Array, endIdx: number) {
  const path: number[] = []
  let p = endIdx
  while (p !== -1) {
    path.push(p)
