...
function Pathfinding(): JSX.Element {
  ...
  const [algo, setAlgo] = useState<'dijkstra' | 'astar' | 'greedy'>('dijkstra')
  ...
  // Search state
  const openRef = useRef(new MinHeap<any>((n) => n.f)) // {x,y,g,f}
  const visitedRef = useRef<Uint8Array>(new Uint8Array(0))
  const visitedAtRef = useRef<Float64Array>(new Float64Array(0))
  const prevRef = useRef<Int32Array>(new Int32Array(0))
  const gScoreRef = useRef<Float32Array>(new Float32Array(0))
  const doneRef = useRef(false)
  const lastStepAt = useRef(0)
  // Precomputed path helpers to reduce per-frame allocations
  const pathPosRef = useRef<Int32Array>(new Int32Array(0)) // index -> position in path or -1
  const pathNextRef = useRef<Int32Array>(new Int32Array(0)) // index -> next index in path or -1

  // Visuals
  const hoverCellRef = useRef<{ x: number; y: number } | null>(null)
  const selectedAtRef = useRef<Float64Array>(new Float64Array(0))
  ...
  const pointerToCell = useCallback((evt: MouseEvent) => pointerToCellFromEvent(evt, canvasRef.current, cellSize, cols, rows), [cellSize, cols, rows])
  ...
  const recomputeGrid = useCallback(() => {
    ...
    openRef.current = new MinHeap<any>((n) => n.f)
  ...
  const resizeCanvas = useCallback(() => {
    const canvas = canvasRef.current
    if (!canvas) return
    resizeCanvasToDpr(canvas, ctxRef as any, recomputeGrid)
  }, [recomputeGrid])
  ...
  const heuristic = useCallback((x: number, y: number) => Math.abs(x - end.x) + Math.abs(y - end.y), [end])
  const pushOpen = useCallback((n: any) => { openRef.current.push(n) }, [])
  const popOpen = useCallback(() => openRef.current.pop(), [])
  const neighbors = useCallback((x: number, y: number) => get4Neighbors(x, y, cols, rows), [cols, rows])
  ...
  const step = useCallback(() => {
    ...
    if (cur.x === end.x && cur.y === end.y) {
      ...
      const path: number[] = []
      let p = endIdx
      while (p !== -1) { path.push(p); const prevVal = prevRef.current[p] ?? -1; p = prevVal }
      path.reverse()
      ...
      for (let k = 0; k < path.length; k++) {
        ...
        const nextIdx = k + 1 < path.length ? (path[k + 1] as number) : -1
        ...
        if (visitedRef.current[i] && grid[i] !== 1) {
          ...
          if (showDirections && cellSize >= 10 && shownPathCount > 0) {
            ...
            if (pos !== -1 && pos + 1 < shownPathCount) {
              ...
              if (nextIdx !== -1 && nextIdx !== undefined) {
          ...
          if (showDistances && cellSize >= 16) {
            const g: number = gScoreRef.current[i] as number
            if (isFinite(g)) drawCenteredText(ctx as any, String(g), Number(cx), Number(cy), Number(cellSize))
          }
        }
      }
    }
  ...
  const onMouseDown = useCallback((e: MouseEvent) => {
    ...
    if ((e as any).shiftKey) { setStart(cell); resetSearch(); return }
    if ((e as any).ctrlKey || (e as any).metaKey) { setEnd(cell); resetSearch(); return }
  ...
  const onMouseMove = useCallback((e: MouseEvent) => {
    ...
    if ((e as any).shiftKey || (e as any).ctrlKey || (e as any).metaKey) return
    const i = index(cell.x, cell.y)
  ...
  useEffect(() => {
    ...
    c.addEventListener('mousemove', onMouseMove as any)
    ...
    return () => {
      ...
      c.removeEventListener('mousemove', onMouseMove as any)
  ...
  const startPause = () => { if (isRunning) { setIsRunning(false) } else if ((openRef.current as any).size === 0 && !doneRef.current) { startSearch() } else { setIsRunning(true) } }
    ...
    <>
      ...
      <div className="control-row">
        <label className="inline">
          Algorithm
          <select value={algo} onChange={(e) => { setAlgo(e.target.value as any); resetSearch() }}>
      ...
      <div className="control-row">
        <label className="inline">
          ...
          <input type="range" min="1" max="240" value={speed} onChange={(e) => setSpeed(parseInt(e.target.value))}
            onWheel={(e) => { e.preventDefault(); const d = (e as any).deltaY > 0 ? -1 : 1; setSpeed(p => Math.max(1, Math.min(120, p + d))) }} />
          <span>{speed} fps</span>
        </label>
      </div>
      <div className="control-row">
        <label className="inline">
          ...
          <input type="range" min="8" max="32" value={cellSize} onChange={(e) => setCellSize(parseInt(e.target.value))}
            onWheel={(e) => { e.preventDefault(); const d = (e as any).deltaY > 0 ? -1 : 1; setCellSize(p => Math.max(8, Math.min(32, p + d))) }} />
